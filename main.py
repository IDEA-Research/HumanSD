...
from pytorch_lightning.callbacks import Callback, BackboneFinetuning, BaseFinetuning, LearningRateMonitor
...
class ImageLogger(Callback):
    ...
    def _testtube(self, pl_module, images, batch_idx, split):
        if hasattr(pl_module.logger.experiment, "add_image") :
            for k in images:
...
# class DDPMFinetuning(BackboneFinetuning):
#     def __init__(self,unfreeze_at_epoch,train_bn):
#         super().__init__()
#         self._unfreeze_at_epoch = unfreeze_at_epoch
#         self.train_bn=train_bn
    
#     def freeze_before_training(self, pl_module):
#         modules = BaseFinetuning.flatten_modules(pl_module.model)
#         for mod in modules:
#             if isinstance(mod, _BatchNorm) and self.train_bn:
#                 BaseFinetuning.make_trainable(mod)
#             else:
#                 BaseFinetuning.freeze_module(mod)
    
#     # def finetune_function(self, pl_module, current_epoch, optimizer, optimizer_idx,initial_denom_lr: float = 10.0,train_bn: bool = True,):
#     #     # When `current_epoch` is 10, feature_extractor will start training.
#     #     if current_epoch == self._unfreeze_at_epoch:
#     #         modules = self.flatten_modules(pl_module)
#     #         for mod in modules:
#     #             params_lr = optimizer.param_groups[0]["lr"]
#     #             denom_lr = initial_denom_lr
#     #             params = self.filter_params(mod, train_bn=train_bn, requires_grad=True)
